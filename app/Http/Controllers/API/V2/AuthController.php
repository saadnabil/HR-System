...
class AuthController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        info($request -> fcm_token );

        /** @var User|Null $user */
        $user = User::where('email',$validatedData['login_id'])->first();
       // dd($validatedData);
        ...
        return $this->success([
            ...
            'week_vacations'      => $weekVacations ? explode(',',$weekVacations->week_vacations) : '',
            'company_start_time'  => company_setting()['company_start_time'],
            'company_end_time'    => company_setting()['company_end_time'],
            'slate'               => $url,
            'token'               => $token,
        ], __('messages.login'));
    }
