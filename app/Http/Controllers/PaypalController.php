        ...
        if($plan)
        {
            try
            {
                ...
                catch(\PayPal\Exception\PayPalConnectionException $ex) //PPConnectionException
                {
                    ...
                    {
                        // dd('1');
                        return redirect()->route('stripe',($plan->id))->with('error', __('Connection timeout'));
                    }
                    else
                    {
                        // dd('2');

                        return redirect()->route('stripe', ($plan->id))->with('error', __('Some error occur, sorry for inconvenient'));
                    }
                }
                ...
                return redirect()->route('payment', encrypt($plan->id))->with('error', __('Unknown error occurred'));
            }
            catch(\Exception $e)
            {
