...
class ReminderEmployeeStartWork extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'reminder:employeestartwork';
    /**
    ...
    public function handle()
    {
        ...
        foreach($companies as $company){
            ...
            if( !in_array(Carbon::now()->format('l') , $week_vacations) &&  $holiday == null && Carbon::now()->gte($start_time) &&  Carbon::now()->lt($end_time)){
                ...
                $attend_today_employees = AttendanceEmployee::where(['created_by'  =>  $company -> id , 'clock_out' => null ])
                                                ->whereDate('created_at' , date('Y-m-d'))
                                                ->pluck('employee_id')
                                                ->toarray();

                $not_attend_today = Employee::where(['created_by'  =>  $company -> id , 'is_active' => 1  ])
                                            ->whereNotIn('id' , $attend_today_employees)
                                            ->pluck('user_id')
                                            ->toarray();
                ...
                $result = pushNotification( __('Start work') , __('We remind you to start your work at ') .' '  .Carbon::now() -> format('H:i a') , null ,  $not_attend_fcm_tokens);
            }
        }
