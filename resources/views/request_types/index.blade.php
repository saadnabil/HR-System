    ...
    <div class="all-button-box row d-flex justify-content-end">
        @can('Create Employee')
            <div class="col-xl-2 col-lg-2 col-md-4 col-sm-6 col-6">
                <a href="#" data-url="{{ route('request_types.create' , ['parent' => request()->get('id')]) }}" class="btn btn-primary" data-ajax-popup="true" data-title="{{__('Create New')}}">
    ...
    <div class="container d-flex flex-column flex-lg-row" id="kt_docs_content_container">
        ...
        <div class="card card-docs flex-row-fluid mb-2">
            ...
            <div class="card-body fs-6 py-15 px-10 py-lg-15 px-lg-15 text-gray-700">
                ...
                <div class="py-0">
                    <!--begin::Heading-->
                    <h1 class="fw-bold mb-5">{{__('Request types')}}</h1>

                    <div class="d-flex align-items-center gap-2 gap-lg-3">
                        <!--begin::Primary button-->
                        @can('Create Employee')
                        @if(isset($parent))
                        <a href="#" data-url="{{ route('request_types.create') }}?id={{ request()->get('id') }}" data-ajax-popup="true"
                                    ...
                                    <table class="table align-middle border rounded table-row-dashed fs-6 g-5" id="kt_datatable_example">
                                        <thead>
                                            <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase">
                                                <th class="min-w-100px">{{__('Name_ar')}}</th>
                                                <th class="min-w-100px">{{__('Name')}}</th>
                                                @if(request()->has('id'))
                                                    <th class="min-w-100px">{{__('Max days')}}</th>
                                                    <th class="min-w-100px">{{ __('Max days per month') }}</th>
                                                    <th class="min-w-100px"> {{ __('The maximum number of hours for submitting the leave after the hour of starting work') }}</th>
                                                @endif
                                                <th class="min-w-100px" width="200px">{{ __('Action') }}</th>
                                            </tr>
                                        </thead>
                                        <tbody class="fw-semibold text-gray-600">
                                            @foreach ($requestTypes as $requestType)
                                                <tr>
                                                    <td>{{ $requestType->title_ar }}</td>
                                                    <td>{{ $requestType->title }}</td>
                                                    @if(request()->has('id'))
                                                        <td>{{ $requestType->maxDays }}</td>
                                                        <td>{{ $requestType->maxDaysPerMonth }}</td>
                                                        <td>{{ $requestType->afterMaxHour }}</td>
                                                    @endif
                                                    <td class="Action text-center">
                                                        <span>
                                                            @if(request()->has('id'))
                                                                @can('Edit Employee')
                                                                    <a href="#" class="btn btn-icon btn-active-light-success w-30px h-30px" data-url="{{ route('request_types.edit', $requestType->id) }}" data-size="lg" data-ajax-popup="true" data-title="{{ __('Edit') }}" data-toggle="tooltip" data-original-title="{{__('Edit')}}"><i class="fa fa-edit"></i></a>
                                                                @endcan
                                                                @can('Delete Employee')
                                                            ...
                                                            @endif
                                                            @if(!request()->has('id'))
                                                            <a href=" {{ route('request_types.index' , ['id' => $requestType->id]) }}" class="btn btn-success btn-sm"  data-toggle="tooltip" data-original-title="{{__('Edit')}}"><i class="fa fa-list"></i> {{ __('Display all types') }}</a>
                                                            @endif
                                                        </span>
                                                    </td>
                                                </tr>
