    ...
    <div class="container d-flex flex-column flex-lg-row" id="kt_docs_content_container">
        ...
        <div class="card card-docs flex-row-fluid mb-2">
            ...
            <div class="card-body fs-6 py-15 px-10 py-lg-15 px-lg-15 text-gray-700">
                ...
                <div class="py-0">
                    <!--begin::Heading-->
                    <h1 class="fw-bold mb-5">{{ __('Structure list') }}</h1>
                                            ...
                                            @foreach($CompanyStructures as $CompanyStructure)
                                                <tr>
                                                    ...
                                                    <td>
                                                        @if($CompanyStructure->employees)
                                                            @foreach($CompanyStructure->employees as $emp)
                                                                <a class="badge badge-light-primary fs-7 m-1" href="{{ route('employee.show', ($emp->id)) }}"><span>{{$emp['name'.$lang]}}</span></a> -
                                                            @endforeach
                                                        @else
                                                            -
                                                        @endif
                                                    ...
                                                    <td class="Action text-center">
                                                        <span>
                                                            @can('Edit Branch')
                                                                <a href="{{asset('companytree/'.$CompanyStructure->id)}}" class="btn btn-icon btn-active-light-secondary w-30px h-30px" data-url="{{ URL::to('companytree/'.$CompanyStructure->id) }}" data-size="lg" data-ajax-popup="true" data-title="" data-toggle="tooltip" data-original-title="{{__('Tree')}}"><i class="fa fa-tree"></i></a>
                                                                <a href="#" class="btn btn-icon btn-active-light-success w-30px h-30px" data-url="{{ URL::to('companystructure/'.$CompanyStructure->id.'/edit') }}" data-size="lg" data-ajax-popup="true" data-title="{{__('Edit Structure')}}" data-toggle="tooltip" data-original-title="{{__('Edit')}}"><i class="fa fa-edit"></i></a>
                                                                <a href="{{asset('companystructure/'.$CompanyStructure->id)}}" class="btn btn-icon btn-active-light-info w-30px h-30px" data-url="{{ URL::to('companystructure/'.$CompanyStructure->id) }}" data-size="lg" data-ajax-popup="true" data-title="" data-toggle="tooltip" data-original-title="{{__('Edit')}}"><i class="fa fa-eye"></i></a>
                                                            @endcan
                                                            @can('Delete Branch')
                                                                <button type="button" class="btn btn-icon confirm-delete btn-active-light-danger w-30px h-30px" data-toggle="tooltip" data-original-title="{{__('Delete')}}" data-confirm="{{__('Are You Sure?').'|'.__('This action can not be undone. Do you want to continue?')}}"><i class="fa fa-trash"></i></button>
            ...
            $('.confirm-delete').click(function(e){
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        document.getElementById('delete-form-{{$CompanyStructure->id}}').submit()
                    }
                });
                return false;
            });
