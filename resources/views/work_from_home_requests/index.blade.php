    ...
    <div class="container d-flex flex-column flex-lg-row" id="kt_docs_content_container">
        ...
        <div class="card card-docs flex-row-fluid mb-2">
            ...
            <div class="card-body fs-6 py-15 px-10 py-lg-15 px-lg-15 text-gray-700">
                ...
                <div class="py-0">
                    <!--begin::Heading-->
                    <h1 class="fw-bold mb-5">{{__('Work from home requests')}}</h1>

                    <div class="d-flex align-items-center gap-2 gap-lg-3">
                        <!--begin::Primary button-->
                        @can('Create Employee')
                        <a href="#" data-url="{{ route('work_from_home_request.create') }}" data-ajax-popup="true"
                                ...
                                <div class="card-body">
                                    ...
                                    <table class="table align-middle border rounded table-row-dashed fs-6 g-5" id="kt_datatable_example">
                                        ...
                                        <tbody class="fw-semibold text-gray-600">
                                            @foreach ($work_from_requests as $work_from_request)
                                                <tr>
                                                    <td>{{ $work_from_request->date }}</td>
                                                    <td>{{ app()->isLocale('en') ? $work_from_request->employee -> name : $work_from_request->employee -> name_ar }}</td>
                                                    <td>{{ $work_from_request->status}}</td>
                                                    <td>{{ $work_from_request->reason}}</td>
                                                    <td class="Action text-center">
                                                        <span>
                                                            @can('Edit Employee')
                                                                <a href="#" class="btn btn-icon btn-active-light-success w-30px h-30px" data-url="{{ route('work_from_home_request.edit', $work_from_request->id) }}" data-size="lg" data-ajax-popup="true" data-title="{{ __('Edit') }}" data-toggle="tooltip" data-original-title="{{__('Edit')}}"><i class="fa fa-edit"></i></a>
                                                            @endcan
                                                            @can('Delete Employee')
                                                                ...
                                                                {!! Form::open(['method' => 'DELETE', 'route' => ['work_from_home_request.destroy', $work_from_request->id],'id'=>'delete-form-'.$work_from_request->id]) !!}
                                                                {!! Form::close() !!}
                                                            @endcan
                                                        </span>
                                                    </td>
