@foreach ($missions as $value => $mission)
    <tr data-row-key="{{ $value }}" class="ant-table-row ant-table-row-level-0">
        ...
        <td class="tooltipS1" data-bs-toggle="offcanvas" data-bs-target="#id{{ $value }}"
            aria-controls="id{{ $value }}">
            <div class="tooltip">{{__('View Details')}}</div>  {{ auth()->user()->employeeIdFormat($mission->employee?->id) }}
        </td>

        <td class="tooltipS1" data-bs-toggle="offcanvas" data-bs-target="#id{{ $value }}"
            aria-controls="id{{ $value }}">
            <div class="tooltip">{{__('View Details')}}</div>{{ app()->isLocale('ar') ? $mission->employee?->name_ar : $mission->employee?->name }}
        </td>

        <td class="tooltipS1" data-bs-toggle="offcanvas" data-bs-target="#id{{ $value }}"
            aria-controls="id{{ $value }}">
            <div class="tooltip">{{__('View Details')}}</div> {{ $mission->employee?->jobtitle ? $mission->employee?->jobtitle['name' . $lang] : 'N/A' }}
        </td>

        <td class="tooltipS1" data-bs-toggle="offcanvas" data-bs-target="#id{{ $value }}"
            aria-controls="id{{ $value }}">
            <div class="tooltip">{{__('View Details')}}</div>{{ $mission->date }}
        </td>

         <td class="tooltipS1" data-bs-toggle="offcanvas" data-bs-target="#id{{ $value }}"
            aria-controls="id{{ $value }}">
            <div class="tooltip">{{__('View Details')}}</div>{{ $mission->start }}
        </td>

         <td class="tooltipS1" data-bs-toggle="offcanvas" data-bs-target="#id{{ $value }}"
            aria-controls="id{{ $value }}">
            <div class="tooltip">{{__('View Details')}}</div>{{ $mission->end }}
        </td>
        ...
        <td class="tooltipS1" data-bs-toggle="offcanvas" data-bs-target="#id{{ $value }}"
            ...
            @endphp
             <div class="buttonS2  {{ $status_class_array[$mission->status] }}">
                {{ $status_translate_array[$mission->status] }}
            </div>
            <div class="tooltip">{{ __('View Details') }}</div>
        </td>
        ...
        <td>
            <div class="actions flex gap-3">
                @if($mission->status == 'pending')
                    @can('Mission-Accept')
                        <a href="{{ route('mission.approve',  $mission ) }}" class="buttonS1 primary">{{ __('Accept') }}</a>
                    @endcan
                    @can('Mission-Reject')
                        <a href="{{ route('mission.reject',  $mission ) }}" class="buttonS1 rejected">{{ __('Reject') }}</a>
                    @endcan
                @endif
            </div>
        </td>
                    ...
                    <div class="collapse show" id="employeeInformation">
                        <div class="cards">
                            <div class="cardS1 flex align">
                                ...
                                <div class="des">{{ app()->isLocale('ar') ? $mission->employee?->name_ar : $mission->employee?->name }}</div>
                            </div>
                            <div class="cardS1 flex align">
                                <div class="name">{{ __('Code') }}</div>
                                <div class="des"> {{ auth()->user()->employeeIdFormat($mission->employee?->id) }}</div>
                            </div>
                            <div class="cardS1 flex align">
                                <div class="name">{{ __('Job Title') }}</div>
                                <div class="des">{{ $mission->employee?->jobtitle ? $mission->employee?->jobtitle['name' . $lang] : 'N/A' }}</div>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="ant-collapse flex align between">
                        <span class="ant-collapse-header-text">{{ __('Request Details') }}</span>
                            ...
                            <div class="flex gap-15">
                                ...
                                @can('Mission-Delete')
                                    <img data-bs-toggle="modal" data-bs-target="#confirm1" class="delete" data-route="{{ route('mission.destroy' , $mission->id) }}"  src="/new-theme/icons/delete.svg" alt="" />
                                @endcan
                            </div>
                ...
                <div class="sectionDDS1 section py-0" style="border: none">
                    <div class="cards">
                        <div class="cardS1 flex align">
                            ...
                            <div class="des">{{ $mission->date }}</div>
                        </div>
                        <div class="cardS1 flex align">
                            <div class="name">{{ __('Reason') }}</div>
                            <div class="des">{{ $mission->reason }}</div>
                        </div>
                         <div class="cardS1 flex align">
                            <div class="name">{{ __('Start') }}</div>
                            <div class="des">{{ $mission->start }}</div>
                        </div>
                        <div class="cardS1 flex align">
                            <div class="name">{{ __('End') }}</div>
                            <div class="des">{{ $mission->end }}</div>
                        </div>
                        <div class="cardS1 flex align">
                            <div class="name">
                            {{ __('Status') }}
                             ...
                             <div class="buttonS2  {{ $status_class_array[$mission->status] }}">
                                {{ $status_translate_array[$mission->status] }}</div>
                            </div>
                    </div>
                </div>
    ...
    <div style="width: 470px;" class="offcanvas offcanvas-end" tabindex="-1" id="edit{{ $value }}"
        ...
        <div class=" drawerS1">
            ...
            <div class="contentDrawer scroll">
                ...
                <div class="sectionDDS1 section py-0" style="border: none">
                    <form class="ajax-submit" action="{{ route('mission.update', $mission) }}" method="post">
                     @csrf
                    @method('put')
                    <div class="cardS1 datePicker my-4">
                        ...
                        <div class="inputS1">
                            <select name="employee_id" required>
                                ...
                                @foreach ($employees as $employee)
                                    <option
                                        {{ $employee?->id ==  $mission->employee_id ? 'selected' : '' }}
                                        value="{{ $employee?->id }}">
                        ...
                        <div class="cardS1 my-4">
                            ...
                            <div class="inputS1">
                                ...
                                <input name="date" type="text" value="{{ Carbon\Carbon::createFromFormat('Y-m-d',$mission->date)->format('d/m/Y')  }}" placeholder="{{ __('Set The Date') }}" name="daterange"
                        ...
                        <div class="cardS1 datePicker my-4">
                                    ...
                                    <div class="inputS1">
                                        ...
                                        <input name="start" value="{{ old('from', $mission->start) }}"
                        ...
                        <div class="cardS1 datePicker my-4">
                                    ...
                                    <div class="inputS1">
                                        <img src="{{ url('new-theme/images/clock.svg') }}" class="iconImg" />
                                        <input name="end" value="{{ old('from', $mission->end) }}"
                                            class="time-pickable" id="startTime" readonly>
                                    </div>
                                </div>
                        ...
                        <div class="cardS1  my-4">
                            ...
                            <div class="inputS1">
                                <input name="reason" type="text" value="{{ $mission->reason }}" placeholder="Set The Reason" />
                            </div>
                        </div>
                        ...
                        <div class="cardS1 my-4">
                            ...
                            <div class="inputS1">
                                <select name="status" id="changeStatus">
                                    <option {{ $mission->status =="approved" ? "selected" : "" }} value="approved">{{ __('Approved') }}</option>
                                    <option  {{ $mission->status =="rejected" ? "selected" : "" }}  value="rejected">{{ __('Rejected') }}</option>
                                </select>
                            </div>
                        </div>
