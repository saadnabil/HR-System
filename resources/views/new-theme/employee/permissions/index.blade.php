        ...
        <div class="pageS1">
            <div class='sectionS2'>
                <div class="head withBorder flex align between options">
                    <h3 class='small'>{{ __('All permisions') }}</h3>
                    <div class='flex align gap-3 options'>
                        @can('EmployeePermission-Print')
                        ...
                        @endcan
                        @can('EmployeePermission-Create')
                            <a href="{{ route('employee-permissions.create') }}">
                ...
                <div class="tableS1 scroll trPointer">
                    <table>
                        ...
                        <tbody>
                            @include('new-theme.employee.permissions.permissions')
                        </tbody>
                    </table>
            ...
            <div class="paginater">
                @include('new-theme.employee.permissions.paginate')
            </div>
        </div>
    </div>
@endsection
...
@push('script')
    @include('new-theme.components.export_print', [
        'print_url' => route('employee_permissions.print'),
        'export_url' => route('employee_permissions.export'),
    ])
    <script>
        $(document).ready(function() {
            function fetch_data(query, start_date, end_date) {
                $.ajax({
                    url: "{{ route('employee-permissions.index') }}",
                    data: {
                        "search": query,
