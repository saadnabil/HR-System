...
@section('content')
    <div class="reportsPage">
        <div class="pageS1">
            ...
            <div class='sectionS2'>
                <div class="head withBorder flex align between">
                    ...
                    <div class='flex align gap-3'>
                        ...
                        @can('EmailReport-Export')
                            <a href="{{ Route('emailreport.export') }}">
                                <button class='buttonS2 withBorder'>
                ...
                <div class="tableS1 scroll">
                    <table>
                        ...
                        <tbody>
                            @include('new-theme.reports.email.emails')
                        </tbody>
                    </table>
                </div>
            </div>
            ...
            <div class="paginater">
                @include('new-theme.reports.email.paginate')
            </div>
...
@push('script')
    <script>
        $(document).ready(function() {
            function fetch_data(query, email_department) {
                $.ajax({
                    url: "{{ route('report.employee_with_emails') }}",
                    data: {
                        "search": query,
                    ...
                    success: function(data) {
                        ...
                        $(document).on('click', '.pagination a', function(event) {
                            ...
                            if (position == -1) {
                                href = page + '?search=' + query + '&department=' +
                                    email_department;
                            } else {
                                href = page + '&search=' + query + '&department=' +
                                    email_department;
            ...
            $(document).on('keyup', '#search', function() {
                var query = $('#search').val();
                var department = $('#email_department').val();
                fetch_data(query, department);
            });

            $(document).on('change', '#email_department', function() {
                var query = $('#search').val();
                var department = $('#email_department').val();
                fetch_data(query, department);
            });
