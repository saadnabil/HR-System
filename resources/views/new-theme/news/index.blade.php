        ...
        <div class="pageS1">
            ...
            <div class='sectionS2'>
                <div class="head  flex align between">
                    <h3 class='small'>@lang('All News')</h3>
                    <div class='flex align gap-3 options'>

                        <div class="searchWithDate m-0">
                            <div class='datePickerContainer'>
                                <div class='datePicker'>
                                    <h3>{{__('Date')}}</h3>
                                    <div class="inputS1">
                                        <img src="/new-theme/icons/date.svg" class="iconImg" />
                                        <input style="width: 225px;" id="datePickerRange" value="" type="text" value=""
                                            placeholder="{{__('Set The Time')}}" name="datepicker" class="datePickerRange"
                                            autocomplete="off" />
                                    </div>
                                </div>
                            </div>
            ...
            <div class="search">
                <div class="inputS1 taskSearch searchInput">
            ...
            <div class="newsCards content py-4">
                <div class="row news-data">
                    @include('new-theme.news.news-data')
                </div>
            </div>
        ...
        $(document).ready(function() {
            function fetch_data() {
                $.ajax({
                    ...
                    data: {
                        "search": $('#search').val(),
                        "datePickerRange"  : $('#datePickerRange').val(),
                    },
                    success: function(data) {
                        $('.news-data').html('');
                        $('.news-data').html(data.search);
                        $(document).on('click', '.pagination a', function(event) {
                            event.preventDefault();
                            var query           = $('#search').val();
                            var datePickerRange = $('#search').val();

                            var page = $(this).attr('href');
                            let position = page.search("&");
                            if (position == -1) {
                                href = page + '?search=' + query+'&datePickerRange=' + datePickerRange;
                            } else {
                                href = page + '&search=' + query+'&datePickerRange=' + datePickerRange;
                            }
            ...
            $(document).on('change', '#datePickerRange', function() {
                fetch_data();
            });
