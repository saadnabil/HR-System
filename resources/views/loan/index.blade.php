        ...
        <div class="container d-flex flex-column flex-lg-row" id="kt_docs_content_container">
            ...
            <div class="card card-docs flex-row-fluid mb-2">
                ...
                <div class="card-body fs-6 py-15 px-10 py-lg-15 px-lg-15 text-gray-700">
                    ...
                    <div class="py-0">
                        <!--begin::Heading-->
                        <h1 class="fw-bold mb-5">{{ __('Loan requests') }}</h1>

                        <div class="d-flex align-items-center gap-2 gap-lg-3">
                            <!--begin::Primary button-->
                            <a href="#" data-url="{{ route('loan.create') }}" data-ajax-popup="true"
                                        ...
                                        <table class="table align-middle border rounded table-row-dashed fs-6 g-5"
                                            ...
                                            <thead>
                                                <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase">
                                                    ...
                                                    <th class="min-w-100px">{{ __('Month number') }}</th>
                                            ...
                                            <tbody class="fw-semibold text-gray-600">
                                                @foreach ($loans as $loan)
                                                    <tr>
                                                        <td>{{ app()->isLocale('en') ? $loan->employee->name : $loan->employee->name_ar }}
                                                        </td>
                                                        <td>{{ $loan->start_date }}</td>
                                                        <td>{{ $loan->amount }}</td>
                                                        <td>{{ $loan->month_nubmer }}</td>
                                                        <td>{{ $loan->reason }}</td>
                                                        <td>
                                                            @if ($loan->status == 'pending')
                                                                ...
                                                                @if ($loan->status == 'Pending')
                                                                    @can('Edit Leave')
                                                                        <a href="#"
                                                                            data-url="{{ URL::to('leave/' . $loan->id . '/edit') }}"
                                                                ...
                                                                @if ($loan->status == 'pending')
                                                                    <a href="#"
                                                                        ...
                                                                        data-action="{{ route('loan_requests.approve', $loan) }}"
                                                                        data-bs-toggle="modal"
                                                                        data-bs-target="#acceptModel"><i
                                                                    ...
                                                                    <a href="#"
                                                                        ...
                                                                        data-action="{{ route('loan_requests.reject', $loan) }}"
                                                                        data-bs-toggle="modal"
                                                                        data-bs-target="#rejectModel"><i
                                                            ...
                                                            @elseif($loan->status == 'rejected')
                                                                <div class="badge badge-pill badge-danger">
                                                                    {{ $loan->status }}</div>
                                                            @elseif($loan->status == 'approvedWithDeduction')
                                                                <div class="badge badge-pill badge-success">
                                                                    {{ $loan->status }}</div>
                                                            @endif
                                                        </td>
                                                        ...
                                                        @if (request('status') == 'approved' || request('status') == 'approvedWithDeduction')
                                                            <td>
                                                                @if ($loan->ticket_flight_status == 'no_both')
                                                                    {{ __('He did not take advantage of the flights') }}
                                                                @elseif($loan->ticket_flight_status == 'go')
                                                                    {{ __('Going only') }}
                                                                @elseif($loan->ticket_flight_status == 'back')
                                                                    {{ __('Back only') }}
                                                                @elseif($loan->ticket_flight_status == 'go_back')
                                                        ...
                                                        <td class="text-right action-btns">
                                                            @if ($loan->status == 'pending')
                                                                <span>
                                                                    ...
                                                                    {!! Form::open([
                                                                        'method' => 'DELETE',
                                                                        'route' => ['loan-requests.destroy', $loan],
                                                                        'id' => 'delete-form-' . $loan,
                                                                    ]) !!}
                                                                    {!! Form::close() !!}
                                                                </span>
