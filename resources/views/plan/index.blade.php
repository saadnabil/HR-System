        ...
        <div class="container d-flex flex-column flex-lg-row" id="kt_docs_content_container">
            ...
            <div class="card card-docs flex-row-fluid mb-2">
                ...
                <div class="card-body fs-6 py-15 px-10 py-lg-15 px-lg-15 text-gray-700">
                    ...
                    <div class="py-0">
                        <!--begin::Heading-->
                        <h1 class="fw-bold mb-5">{{__('Manage Plan')}}</h1>
                                        ...
                                        <table class="table align-middle border rounded table-row-dashed fs-6 g-5" id="kt_datatable_example">
                                            ...
                                            <tbody class="fw-semibold text-gray-600">
                                                @foreach($plans as $plan)
                                                    <tr>
                                                        <td>{{$plan->name}}</td>
                                                        <td>{{(!empty(env('CURRENCY_SYMBOL'))?env('CURRENCY_SYMBOL'):'$').$plan->price}}</td>
                                                        <td>{{ ucfirst($plan->duration)}}</td>
                                                        <td> {{($plan->max_users==-1)?__('Unlimited'):$plan->max_users}}</td>
                                                        <td>{{($plan->max_employees==-1)?__('Unlimited'):$plan->max_employees}}</td>

                                                        <td class="Action text-center">
                                                            <span>
                                                                @can('Edit Plan')
                                                                    <a href="#" class="btn btn-icon btn-active-light-success w-30px h-30px" data-url="{{ route('plans.edit',$plan->id) }}" data-size="lg" data-ajax-popup="true" data-title="{{ __('Edit Timesheet') }}" data-toggle="tooltip" data-original-title="{{__('Edit')}}"><i class="fa fa-edit"></i></a>
                                                                @endcan
                                                            </span>
                                                        </td>
                                                    </tr>
