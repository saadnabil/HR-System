                                        ...
                                        <table class="table align-middle border rounded table-row-dashed fs-6 g-5" id="kt_datatable_example">
                                            ...
                                            <tbody class="fw-semibold text-gray-600">
                                                @foreach($users as $user)
                                                    <tr>
                                                        <td>
                                                            <img style="width: 100px;" alt="{{ $user->name }}" src="{{ $user->avatar ? $profile.'/'.auth()->user()->avatar : $profile.'/avatar.png'}}">
                                                        </td>
                                                        <td>{{ $user->name }}</td>
                                                        <td>{{ $user->email }}</td>
                                                        <td>{{ $user->type }}</td>
                                                        @if(auth()->user()->type == 'super admin')
                                                            <td>{{auth()->user()->countUsers()}}</td>
                                                            <td>{{auth()->user()->countEmployees()}}</td>
                                                        ...
                                                        <td class="Action text-center">
                                                            <span>
                                                                @can('Show Employee Profile')
                                                                    <a href="{{route('employee.show',\Illuminate\Support\Facades\Crypt::encrypt($user->employee->id))}}" class="btn btn-icon btn-active-light-infi w-30px h-30px" title="{{__('Show User')}}"><i class="fa fa-eye"></i></a>
                                                                @endcan
                                                                @can('Edit User')
                                                                    <a href="#" class="btn btn-icon btn-active-light-success w-30px h-30px" data-url="{{ route('user.edit', $user->id) }}" data-size="lg" data-ajax-popup="true" data-title="{{ __('Edit User') }}" data-toggle="tooltip" data-original-title="{{__('Edit')}}"><i class="fa fa-edit"></i></a>
                                                                @endcan
                                                                @can('Delete User')
                                                                    <button type="button" class="btn btn-icon confirm-delete btn-active-light-danger w-30px h-30px" data-toggle="tooltip" title="{{__('Delete')}}"><i class="fa fa-trash"></i></button>
                                                                ...
                                                                @if(auth()->user()->type == 'super admin')
                                                                    <div class="col-6 text-center Id">
                                                                        <a href="#" data-url="{{ route('plan.upgrade',$user->id) }}" class="btn btn-icon btn-active-light-success" data-ajax-popup="true" title="{{__('Upgrade Plan')}}">
