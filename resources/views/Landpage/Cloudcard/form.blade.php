...
<div class="card shadow-sm">
    <div class="card-body">
        ...
        <form method="POST" action="{{ $case == 'update' ? route('landpage.landcloudcard.update', ['id' => $row->id]) : route('landpage.landcloudcard.store') }}" enctype="multipart/form-data">
            {{ csrf_field() }}
            <div class="row">
                <div class="col-md-12">
                    <div class="mb-10">
                        <label for="number" class="required form-label">{{ __('landpage.Percentage') }}</label>
                        <input name="number" value="{{ $case == 'update' ? $row->number : old('number') }}" id="number" type="number" min="1" max="100" class="form-control"  />
                        @error('number')
                            <label for="number" style="color:#f00;font-size:12px;margin-top:3px;">{{ $message }}</label>
                        @enderror
                    </div>
                </div>
                <div class="col-md-12" >
