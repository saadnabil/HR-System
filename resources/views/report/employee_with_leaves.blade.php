    ...
    <div class="container d-flex flex-column flex-lg-row" id="kt_docs_content_container">
        ...
        <div class="card card-docs flex-row-fluid mb-2">
            ...
            <div class="card-body fs-6 py-15 px-10 py-lg-15 px-lg-15 text-gray-700">
                ...
                <div class="py-0">
                    <!--begin::Heading-->
                    <h1 class="fw-bold mb-5">{{__('Employee with leaves')}}</h1>
                    <!--end::Heading-->
                    <!--begin::Block-->
                    <div class="py-5">
                        <!--begin::Card-->
                                    ...
                                    <table class="table align-middle border rounded table-row-dashed fs-6 g-5" id="kt_datatable_example">
                                        <thead>
                                            <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase">
                                                ...
                                                <th class="min-w-100px">{{ __('Leave credit') }}</th>
                                                @foreach ($leaveTypes as $leaveType)
                                                <th class="min-w-100px">{{ app()->isLocale('en') ?  $leaveType->title : $leaveType->title_ar }}</th>
                                                @endforeach
                                            </tr>
                                        </thead>
                                        <tbody class="fw-semibold text-gray-600">
                                            @foreach ($employees as $employee )
                                                <tr class="">
                                                    <td class="font-style">{{ app()->isLocale('en') ? $employee->name : $employee->name_ar }}</td>
                                                    <td>{{ $employee-> annual_leave_entitlement}} {{ __('Days') }}</td>
                                                    @foreach ($leaveTypes as $leaveType)
                                                    <td>{{ $employee->getCurrentYearLeaves()->where('leave_type_id' ,$leaveType->id)->sum('total_leave_days')}} {{ __('Days') }}</td>
                                                    @endforeach
                                                </tr>
                                            @endforeach
