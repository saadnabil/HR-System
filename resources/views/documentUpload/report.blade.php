                                    ...
                                    <table class="table align-middle border rounded table-row-dashed fs-6 g-5" id="kt_datatable_example">
                                        <thead>
                                            <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase">
                                                ...
                                                <th class="min-w-100px">{{ app()->isLocale('en') ?  $document->name :  $document->name_ar}}</th>
                                                @endforeach
                                                <th class="min-w-100px">{{__('Upload percentage')}}</th>
                                            </tr>
                                        </thead>
                                        <tbody class="fw-semibold text-gray-600">
                                            @foreach ($employees as $employee )
                                                <tr>
                                                    <td>{{ $employee->name }}</td>
                                                    @foreach ($documents as $document )
                                                        <td>{{ $employee->ducument_uploads->where('document_id' ,$document->id )->first() != null ? 'Done' : 'False' }} </td>
                                                    @endforeach
                                                    <td>{{ round(count($documents) > 0 ?  $employee->ducument_uploads->count() / count($documents) *100 : 0 )}} %</td>
                                                </tr>
                                                @endforeach
                                        </tbody>
                                    </table>
